...
%% GLOBAL SET-UP & DEFINITION
    % t_in = currently wrong
    % t_out = t_in in A* search.
...
%     least_time_collision = 2.5;
    time_2_travel_2_roads = 2.5;
    least_time_collision = 1.5;
...
%             elseif ( abs(t_out-corruptWindow(i,7)) <= (time_AGV2_turn+least_time_collision)) 
...
%             end
    ...
    % IF there is a turning of one or another AGV
    % => Wait for AGV_2 to turn and cross first( since AGV_1 is planned after)
            if ( abs(t_out-corruptWindow(i,7)) <= (time_AGV2_turn+least_time_collision)) 
%             elseif ( abs(t_out-corruptWindow(i,7)) <= (time_AGV2_turn+time_2_travel_2_roads)) 
                type = 3;  
                time = time_AGV2_turn + time_2_travel_2_roads;
            end
...
%             elseif ( abs(t_out-(corruptWindow(i,7))) <= (least_time_collision+time_AGV2_turn))
...
%         end
        ...
        if ( abs(t_out-(corruptWindow(i,7))) <= (least_time_collision + time_AGV2_turn + time_AGV1_turn))
%             elseif ( abs(t_out-(corruptWindow(i,7))) <= (time_2_travel_2_roads+time_AGV2_turn))             
            type = 3; % slow down 
            time = (time_2_travel_2_roads + time_AGV2_turn );
        end
