...
%% Case khac huong   
        if (NodedY~= corruptWindow(i,8) || NodedX~= corruptWindow(i,9))  % d1 ~= d2            
            ...
            if (t_in<=  corruptWindow(i,7) && corruptWindow(i,7)<= t_out+corruptWindow(i,7)-corruptWindow(i,6)) 
                % Neu AGV1 den sau
                type = 3;  
                time = timeOutprior - t_in + (corruptWindow(i,7) -corruptWindow(i,6));
            elseif (corruptWindow(i,7) <= t_in) && (t_in<= (timeOutprior+corruptWindow(i,7)-corruptWindow(i,6)))           
                % Neu AGV2 den sau
                type = 3; % slow down 
                time = timeOutprior - t_in + (corruptWindow(i,7) -corruptWindow(i,6));
            end
...
%             if (corruptWindow(i,7) <= t_in) && (t_in<= corruptWindow(i,7) + 1.5+ time2turn) || (t_in <= corruptWindow(i,7) && corruptWindow(i,7)<= t_in +1.5+time2turn)           
            if (corruptWindow(i,7) <= t_in) && (t_in<= timeOutprior)
                % Neu AGV khong uu tien vao sau => wait                  
                type = 3; % slow down 
                time = timeOutprior - t_in + (corruptWindow(i,7) -corruptWindow(i,6))  ;
                  
            elseif (t_in <= corruptWindow(i,7) && corruptWindow(i,7)<= t_out)               
